...
GROQ_MODEL = "llama3-70b-8192"  # Or your preferred Groq model
...
class PromptAgent:
    def generate_prompt(self, buggy_code, test_code, program_name, analysis):
        ...
        return f"""
You are an expert Python developer and code fixer. Your job is to automatically correct a single-line bug in a classic algorithm implementation. 

Instructions:
- Carefully read the bug analysis and the provided buggy code.
- Fix the bug in the code, making only the minimal change necessary to pass the test cases.
- Do not change the function signature or add extra functions unless required by the bug analysis.
- Ensure the output matches the test case expectations exactly.{output_hint}
