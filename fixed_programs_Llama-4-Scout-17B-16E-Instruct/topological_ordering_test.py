...
def main():
    ...
    eleven.outgoing_nodes = [two_wikipedia, nine, ten]
    eight.incoming_nodes = [seven, three]
    eight.outgoing_nodes = [nine]
    two_wikipedia.incoming_nodes = [eleven]
    nine.incoming_nodes = [eleven, eight]
    ten.incoming_nodes = [eleven, three]

    try:
        [print(x.value, end=" ") for x in topological_ordering([five, seven, three, eleven, eight, two_wikipedia, nine, ten])]
    ...
    five_geekforgeeks = Node(5)
    zero = Node(0)
    four = Node(4)
    one = Node(1)
    two_geekforgeeks = Node(2)
    three = Node(3)

    five_geekforgeeks.outgoing_nodes = [two_geekforgeeks, zero]
    four.outgoing_nodes = [zero, one]
    two_geekforgeeks.incoming_nodes = [five_geekforgeeks]
    two_geekforgeeks.outgoing_nodes = [three]
    zero.incoming_nodes = [five_geekforgeeks, four]
    one.incoming_nodes = [four, three]
    three.incoming_nodes = [two_geekforgeeks]
    three.outgoing_nodes = [one]

    try:
        [print(x.value, end=" ") for x in topological_ordering([four, five_geekforgeeks, zero, two_geekforgeeks, three, one])]
    except Exception as e:
        print(e)
    print()
